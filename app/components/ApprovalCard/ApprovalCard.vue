...
const emit = defineEmits<{ decide: [requestId: string, approved: boolean]; respond: [requestId: string, response: unknown] }>()
const answer = ref('')
const score = ref(5)

function submitInput() {
  if (props.request.kind === 'evaluation') emit('respond', props.request.id, { score: score.value, comment: answer.value.trim() })
  else if (answer.value.trim()) emit('respond', props.request.id, answer.value.trim())
}
</script>
  ...
  <article class="approval-card">
    ...
    <div class="approval-copy">
      <div class="approval-title">
        {{ request.kind === 'approval' || !request.kind ? '需要你的确认' : request.kind === 'evaluation' ? '评价本次结果' : '需要补充信息' }}
        <NTag size="small" :bordered="false">{{ request.risk }}</NTag>
      </div>
      <p>{{ request.question || request.action }}</p><p v-if="request.details">{{ request.details }}</p>
      <div v-if="request.kind === 'clarification'" class="input-row">
        <NInput v-model:value="answer" placeholder="输入补充信息" @keydown.enter.prevent="submitInput" />
        <NButton type="primary" :disabled="!answer.trim()" @click="submitInput">继续</NButton>
      </div>
    ...
    <div v-if="request.kind === 'approval' || !request.kind" class="approval-actions">
...
.approval-actions,.input-row { display: flex; gap: 8px; }
.input-row { margin-top: 10px; }
@media (max-width: 680px) { .approval-card { flex-wrap: wrap; } .approval-actions { width: 100%; justify-content: flex-end; } .input-row { flex-wrap: wrap; } }
</style>
